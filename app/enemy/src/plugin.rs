...
use tropical_fox_combat::{DamageEvent, DeathEvent, EnemyHealth, Health, Invincibility};
use tropical_fox_common::{
    CharacterAssets, CharacterTextureAtlas, Collider, GameState, Gravity, Ground, Player, Velocity,
};
...
use crate::config::{
    AIBehaviorType, EnemiesConfig, FlyingPatternConfig, load_enemies_config_optional,
};
...
/// Load enemy character assets from configuration
fn load_enemy_assets(
    ...
    for (enemy_id, enemy_def) in &enemies_config.enemies {
        ...
        let anim_config = match load_animation_config_optional(&animation_config_path) {
            ...
            None => {
                warn!(
                    "Could not load animation config for enemy '{}' at {}",
                    enemy_id, animation_config_path
                );
                continue;
            }
        };
        ...
        character_assets.insert(enemy_id, CharacterTextureAtlas::new(texture, layout_handle));

        info!(
            "Loaded enemy character '{}': {}x{} sprites, {}x{} grid",
            enemy_id, sprite_size.x, sprite_size.y, anim_config.columns, anim_config.rows
        );
    }

    // Load boss assets
    use crate::config::load_bosses_config_optional;
    let bosses_config = load_bosses_config_optional("assets/config/bosses.ron");

    for (boss_id, boss_def) in &bosses_config.bosses {
        let animation_config_path = format!("assets/{}", boss_def.animation_config_path);
        ...
        let anim_config = match load_animation_config_optional(&animation_config_path) {
            ...
            None => {
                warn!(
                    "Could not load animation config for boss '{}' at {}",
                    boss_id, animation_config_path
                );
                continue;
            }
        };
        ...
        // Insert into character assets
        character_assets.insert(boss_id, CharacterTextureAtlas::new(texture, layout_handle));

        info!(
            "Loaded boss character '{}': {}x{} sprites, {}x{} grid",
            boss_id, sprite_size.x, sprite_size.y, anim_config.columns, anim_config.rows
        );
    }
}
...
impl Plugin for EnemyPlugin {
    fn build(&self, app: &mut App) {
        ...
        // Load enemy character assets in PostStartup
        // This ensures AnimationPlugin's Startup system has created CharacterAssets resource
        app.add_systems(PostStartup, load_enemy_assets);
        ...
        app.add_systems(
            Update,
            enemy_death_system
                .after(tropical_fox_combat::death_system)
                .run_if(in_state(GameState::InGame)),
        );
    }
}
    ...
    if let Some(assets) = character_assets {
        if let Some(atlas) = assets.get(enemy_type) {
            ...
            let (animation_controller, animation_state) = {
                ...
                if !config.animation_config_path.is_empty() {
                    let animation_config_path = format!("assets/{}", config.animation_config_path);

                    if let Some(anim_config) =
                        load_animation_config_optional(&animation_config_path)
                    {
                        match AnimationController::try_from(anim_config) {
                            Ok(controller) => {
                                info!(
                                    "Loaded enemy '{}' animations from {} ({} clips)",
                                    enemy_type,
                            ...
                            Err(e) => {
                                warn!(
                                    "Failed to create animation controller for '{}' from {}: {}. Using fallback.",
                                    enemy_type, animation_config_path, e
                                );
                        ...
                        warn!(
                            "Could not load animation config for '{}' at {}. Using fallback.",
                            enemy_type, animation_config_path
                        );
                        create_fallback_enemy_animation()
                    }
                } else {
